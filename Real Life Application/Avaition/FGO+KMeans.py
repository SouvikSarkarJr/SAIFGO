...
def xie_beni_index(X, labels, centers):
    ...
    for i in range(len(centers)):
        ...
        if len(cluster_points) > 0:
            sse += np.sum(cdist(cluster_points, [centers[i]], metric='sqeuclidean'))
    ...
    return sse / denominator if denominator > 0 else np.inf
...
# FGO Optimization 
print(f"\n--- Starting FGO Optimization (Total Iterations: {max_iter}) ---")
for t in range(max_iter):
    ...
    for i in range(pop_size):
        ...
        # Evaluate fitness for the new generated structures
...
XB_score = xie_beni_index(data_sample, labels_sample, kmeans_centers)
...
ax.set_title("Domestic Flight Network and Airport Across India (using FGO+KMeans)", 
             fontsize=20, fontweight="bold", pad=15)
fig.text(0.5, 0.05, "(b) Aviation Network Using FGO+K-Means", 
...
output_path = os.path.join(output_dir, "FGO_KMeans.png")
