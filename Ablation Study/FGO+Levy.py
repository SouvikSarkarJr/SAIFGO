...
max_iter = 100
beta = 2
alpha = 0.01  # Step size scaling factor for Lévy Flight
random.seed(42)
np.random.seed(42)
...
# Lévy Flight function matching your cluster dimensional shape
...
# --- ABLATION CONFIGURATION: STANDARD FGO + LÉVY ONLY ---
dim = data.shape[1]
lb = np.min(data, axis=0)
ub = np.max(data, axis=0)

# 1. Standard Random Initialization (OBL Removed)
population = []
...
for t in range(max_iter):
    ...
    for i in range(pop_size):
        ...
        # 2. Combined Standard Differential Step + Lévy Flight Mutation
        j, k = np.random.choice(pop_size, 2, replace=False)
        
        # Base exploratory step modified with alpha scaled Lévy distribution jumps
        new_solution = Xi + R * np.random.rand() * (population[j] - population[k]) + alpha * levy_flight((n_clusters, dim))
...
print("\n========== ABLATION STUDY RESULTS (Standard-FGO + Lévy) ==========")
