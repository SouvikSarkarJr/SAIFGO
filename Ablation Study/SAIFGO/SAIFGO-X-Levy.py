...
csv_path = "/Users/souviksarkarjr./Downloads/Dataset/Wave.csv"   # CSV File Path
n_clusters = 3
pop_size = 20          # P in the text
max_iter = 30          # T in the text
beta = 2               # Non-linear decay exponent
r_max = 1.0            # Adaptive control bounds
r_min = 0.0
epsilon = 1e-4         # Baseline probability exploration factor
fuzzifier_m = 2.0      # FCM parameter
...
def assign_clusters(data, Z):
    """Maps to the Cluster Assignment Mechanism using squared Euclidean distance."""
    centroids = Z.reshape(n_clusters, dim)
    distances = cdist(data, centroids, metric='sqeuclidean')
    return np.argmin(distances, axis=1)

def proxy_fitness(Z):
    """Maps to F_proxy using standard Euclidean distance."""
    centroids = Z.reshape(n_clusters, dim)
    distances = cdist(data, centroids, metric='euclidean')
    return -np.sum(np.min(distances, axis=1))

def fast_dunn_index(data, labels):
    """Vectorized calculation matching the single-linkage Dunn Index formulation."""
    ...
    # Calculate Max Intra-cluster diameter (Denominator)
    max_diameter = 0
    ...
    # Calculate Min Inter-cluster distance (Numerator via Single-Linkage)
...
def fast_xie_beni_index(data, labels, Z):
    """Vectorized calculation matching the corrected Xie-Beni Index formulation."""
    centroids = Z.reshape(n_clusters, dim)
    numerator = 0
    for i in range(n_clusters):
    ...
    # Pairwise centroid distances (j != k separation verification)
...
def hybrid_local_refinement(Z_best):
    centroids = Z_best.reshape(n_clusters, dim).copy()
    labels = assign_clusters(data, Z_best)
    
    # 1. Discrete K-Means Step
    for j in range(n_clusters):
        cluster_points = data[labels == j]
        if len(cluster_points) > 0:
            centroids[j] = np.mean(cluster_points, axis=0)
            
    # 2. Fuzzy C-Means Warm-Start Step
    distances = cdist(data, centroids, metric='euclidean')
    distances = np.fmax(distances, 1e-6)
    
    power = 2.0 / (fuzzifier_m - 1)
    inv_dist_powered = (1.0 / distances) ** power
    U = inv_dist_powered / np.sum(inv_dist_powered, axis=1, keepdims=True)
    ...
    for j in range(n_clusters):
        if denominator[j] != 0:
            centroids[j] = np.sum(U_m[:, j].reshape(-1, 1) * data, axis=0) / denominator[j]
            
    return np.clip(centroids.flatten(), LB, UB)
...
# Reduced strategies pool from 3 to 2
strategy_success = np.ones(2)
...
for t in range(max_iter):
    ...
    # Pre-generate strategies and random choice items to save loop runtime (Now only 2 choices)
    strategies = np.random.choice(2, size=pop_size, p=probs)
    ...
    # Hybrid local refinement step
    best_Z = hybrid_local_refinement(best_Z)
    best_score = composite_fitness(best_Z, norm_bounds)
    
    worst_idx = np.argmin(fitness)
    population[worst_idx] = best_Z.copy()
    fitness[worst_idx] = best_score
    
    strategy_success = np.maximum(strategy_success * 0.9, epsilon)

# --- FINAL METRIC PERFORMANCE REPORTING ---
final_labels = assign_clusters(data, best_Z)
s_score, db_score, ch_score, dunn, xb = compute_raw_indices(best_Z)

print("\n========== SAIFGO FINAL METRICS ==========")
print("Optimized Maximized Composite Score:", best_score)
