# ==========================================================
# SAIFGO + FCM + WILCOXON SIGNED-RANK TEST + BAR GRAPH
# ==========================================================
...
if __name__ == "__main__":
    ...
    if not os.path.exists(file_path):
        ...
        for i in range(len(features)):
            # Handle cases where all values in the truncated cluster are identical
            try:
                stat, p_value = wilcoxon(cluster_0[:, i], cluster_1[:, i])
            except ValueError:
                stat, p_value = 0, 1.0  # Zero difference
        ...
        plt.title("Wilcoxon Signed-Rank Test Across Clusters (SAIFGO+FCM)")
        plt.xlabel("Features")
        plt.ylabel("P-values")
        plt.tight_layout()

        output_file = os.path.join(os.getcwd(), "SAIFGO+FCM_Wilcoxon.jpg")
