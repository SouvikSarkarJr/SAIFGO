# ======================================================
#   SAIFGO + KMeans + KRUSKAL-WALLIS + VARGHA-DELANY A12
# ======================================================
    ...
    if not os.path.exists(file_path):
        ...
        if len(effect_matrices) == 0:
            ...
            for idx, (feature_name, matrix) in enumerate(effect_matrices):
                ...
                plt.colorbar()
                plt.title(f"Vargha-Delaney A12 Effect Size - {feature_name} (SAIFGO+KMeans)")
                plt.xticks(range(len(clusters)), [f"C{c}" for c in clusters])
                plt.yticks(range(len(clusters)), [f"C{c}" for c in clusters])

            plt.tight_layout()
            output_file = os.path.join(os.getcwd(), "SAIFGO+KMeans_A12.jpg")
