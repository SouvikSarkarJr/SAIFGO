# ==========================================================
# SAIFGO + FCM + FRIEDMAN TEST + JPG SAVE (UPDATED)
# ==========================================================
...
if not os.path.exists(csv_path) and csv_path != "#": 
    print("Warning: Dataset not found. Please update csv_path.")
...
try:
    ...
    print("Using random data for demonstration. Update csv_path for real usage.")
    df = pd.DataFrame(np.random.rand(150, 4), columns=['F1', 'F2', 'F3', 'F4'])
...
# ======================================================
# FRIEDMAN TEST & GRAPH GENERATION
# ======================================================
clusters = np.unique(final_labels)

if len(clusters) < 3:
    print("Warning: Friedman Test requires at least 3 clusters. Generating graph with available data.")
...
for i in range(len(features)):
    cluster_data = []
    for c in clusters:
        cluster_values = data[final_labels == c, i]
    ...
    if len(clusters) >= 3 and min_size > 0:
        stat, p_value = friedmanchisquare(*cluster_data)
    else:
        stat, p_value = 0, 1.0 # Default fallback
        
    p_values.append(p_value)

    print(f"Feature: {features[i]} | P-value = {p_value:.6f}")
...
plt.title("Friedman Test Across Clusters (SAIFGO+FCM)")
plt.xlabel("Features") 
plt.ylabel("P-values")
plt.tight_layout()
...
print(f"\nImage successfully saved at: {save_path}")
