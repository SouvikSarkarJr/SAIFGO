# ==========================================================
# SAIFGO + FCM + KRUSKAL-WALLIS + MEDIAN & IQR
# ==========================================================
...
class SAIFGO_FCM_Clustering:
    ...
    def fit(self, X):
        ...
        lb_base = np.min(X, axis=0)
        ub_base = np.max(X, axis=0)
        LB = np.tile(lb_base, self.n_clusters)
        UB = np.tile(ub_base, self.n_clusters)
...
# ==========================================================
# 3️⃣ MAIN EXECUTION (STATS & PLOTS)
# ==========================================================
...
if __name__ == "__main__":
    ...
    if not os.path.exists(file_path):
        ...
        # Using MinMaxScaler to conform to the SAIFGO bounds [0, 1]
        scaler = MinMaxScaler()
        X_scaled = scaler.fit_transform(X)

        model = SAIFGO_FCM_Clustering(n_clusters=3, pop_size=20, max_iter=30)
        model.fit(X_scaled)
        ...
        if len(results) == 0:
            ...
            for idx, (feature_name, medians, iqrs) in enumerate(results):
                ...
                plt.title(f"Median ± IQR - {feature_name} (SAIFGO+FCM)")
                plt.ylabel("Median Value")

            plt.tight_layout()
            save_path = os.path.join(os.getcwd(), "SAIFGO+FCM_Stats.jpg")
