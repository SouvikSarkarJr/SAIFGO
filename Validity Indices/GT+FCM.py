# ==========================================================
# GENERAL TREND OPTIMIZER (GT) + FUZZY C-MEANS (FCM)
# + 5 VALIDITY INDICES
# ==========================================================
...
# Initialize personal trend drift matrices for all tracking elements
...
for iteration in range(max_iter):
    ...
    # Calculate global group mean position to form the macroscopic swarm general trend
    ...
    for i in range(n_agents):
        ...
        # Micro-adjustment noise vector for stochastic exploration escape mechanics
        exploration_noise = np.random.randn(n_clusters, n_features) * 0.02
        
        # Mathematical derivation of global group trend vector field pull
        global_trend_vector = w_global * r1 * (global_best - global_mean_position)
        
        # Position translation via personal historical profile and collective trajectory
        ...
        if new_score < fitness_scores[i]:
            # Retain drift vector profile for trend formulation
            trends[i] = new_position - agents[i]
            agents[i] = new_position
            fitness_scores[i] = new_score
        else:
            # Dampening trend vector if the step did not yield improvement
            trends[i] *= 0.5
...
# ==========================================================
# 🔹 5. FINAL FCM USING GT-OPTIMIZED CENTROIDS
# ==========================================================
