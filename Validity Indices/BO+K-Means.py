# ==========================================================
# BILLIARDS OPTIMIZATION (BO) + K-MEANS
...
# 🔹 1. SETTINGS
# ==========================================================
file_path = "/Users/souvik/Documents/Project-II/Data-sets/Bank.csv"
n_clusters = 3
...
for iteration in range(max_iter):
    ...
    for i in range(n_agents):
        ...
        # Random direction and velocity (ball movement)
        direction = np.random.randn(n_clusters, n_features)
        velocity = np.random.rand()

        new_position = agents[i] + velocity * direction

        lower = X_scaled.min(axis=0)
        upper = X_scaled.max(axis=0)

        # Reflection boundary handling
        new_position = np.where(new_position < lower,
                                lower + (lower - new_position),
                                new_position)

        new_position = np.where(new_position > upper,
                                upper - (new_position - upper),
                                new_position)
...
# ==========================================================
# 🔹 8. FINAL K-MEANS USING BO CENTROIDS
# ==========================================================

final_centroids, labels = kmeans_from_centroids(X_scaled, best_solution)
